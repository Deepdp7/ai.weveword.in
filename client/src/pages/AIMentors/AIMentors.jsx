...
export default function AIMentors() {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const { data } = await axios.get(`http://${window.location.hostname}:5000/api/ai/history/${activeMentor}`, {
  ...
  const handleSend = async (e, useCredit = false, retryMessages = null) => {
    ...
    try {
      const response = await axios.post(`http://${window.location.hostname}:5000/api/ai/chat`, {
  ...
  const clearChat = async () => {
    try {
      await axios.delete(`http://${window.location.hostname}:5000/api/ai/history/${activeMentor}`, {
        withCredentials: true
      });
